...
function loadAll() {
  ...
  setTimeout(function() {
    debug("Schaue nach, ob das dein erster das erste mal ist, dass du auf den Server joinst.. " + downloadingFileCalled);
    if (downloadingFileCalled) {
      announce(
        "Bitte warte, bis alle Inhalt geladen wurden!",
        true
      );
    }
  }, 10000);
...
$(document).ready(function() {
  ...
  setTimeout(function() {
    if (!isGmod) {
      debug("Testet Garry's Mod nicht..");
      isTest = true;
      loadAll();
